...
import os
from other_time_schedule import get_other_schedule

####################################
# 这个主要是从sd3生成的图像出发来恢复，看一看效果。
####################################
...
output_dir = "outputs/SD3/generation"
file_list = [os.path.join(output_dir, f) for f in os.listdir(output_dir) if f.endswith(".png")]


# for prompt in prompt_list:
for file in file_list:
    prompt = file.split("/")[-1][:-len(".png")]
    print(prompt)
    ...
    image_ori = Image.open(file).resize((height,width))
    ...
    timesteps = get_other_schedule(num_inference_steps, (768//16 * 1360//16))
    timesteps = torch.flip(timesteps,dims=[0])
    print(timesteps)
    ...
    output_dir = f"outputs/Flux/start_from_sd3_image_fixedvsother_lin_inferences_{num_inference_steps}_cfg_{guidance_scale}_res_{height}_{width}"
